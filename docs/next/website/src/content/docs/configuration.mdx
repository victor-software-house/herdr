...
A machine-local config can inherit one parent with a top-level `extends` value:

```toml
extends = "../shared/herdr.toml"

[ui]
sidebar_width = 28
```

Relative parent paths resolve from the file that declares them. `~` and `~/...`
resolve from your home directory. Each parent may extend one more parent, up to
16 files in the complete chain. Missing files, invalid TOML, unsupported
`~user` paths, cycles, and deeper chains reject the complete load with a
source-file diagnostic.

Herdr merges the oldest parent first and the selected config last. TOML tables
merge recursively. A child scalar, array, array of tables, or value of another
type replaces the parent value. The `extends` key controls loading only; it is
not part of the effective runtime config.
...
Reload reads the complete inheritance chain again. A broken parent keeps the
current running config, just like an invalid selected config. Settings screens,
`config set-channel`, onboarding, and `config reset-keys` write only the
selected child file. They never modify a parent or copy inherited values into
the child. Removing a child override therefore reveals the inherited value on
the next reload. `config reset-keys` cannot remove keybindings declared by a
parent; override them in the child or edit that parent directly.
...
The desktop tab strip preserves upstream spacing by default and exposes its
bounded layout and controls:
...
`min_tab_width` accepts 1–64. Per-side label padding accepts 0–32, and
`label_alignment` is `"left"`, `"center"`, or `"right"`. `gap` controls empty
columns between tab hit rectangles. Scroll and new-tab control widths accept
1–16; each printable label must fit its configured hit width.
`overflow_indicator` and `drop_indicator` must each occupy exactly one terminal
column. `status_gap` reserves 0–16 blank columns before the right-side status
area. When status would leave less than one minimum tab plus both scroll
controls and the new-tab control, tabs keep the complete row and the status
area yields.
...
The compact desktop sidebar keeps the upstream four-column rail by default. Its
rightmost column remains reserved as the boundary before pane content, leaving
three columns for each compact row:
...
`width` accepts 2–32 columns. Each row type independently places its numeric
index and one-column status icon inside the usable `width - 1` columns.
`number_width = 0` hides the number. Otherwise the number range and status
column cannot overlap or cross the usable boundary. `divider` and `toggle` must
each be printable text exactly one terminal column wide.

Workspace and Agent hit targets still cover their complete configured row. The
toggle hit remains centered in the usable rail. `sidebar_collapsed_mode =
"hidden"` still uses zero width and ignores this compact geometry.
