...
fn plugin_link(args: &[String]) -> std::io::Result<i32> {
    let Some(path) = args.first() else {
        super::eprint_usage("plugin link <path> [--disabled]");
        return Ok(2);
    };
...
fn plugin_config_dir_command(args: &[String]) -> std::io::Result<i32> {
    let Some(plugin_id) = args.first() else {
        super::eprint_usage("plugin config-dir <plugin_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        super::eprint_usage("plugin config-dir <plugin_id>");
...
fn plugin_unlink(args: &[String]) -> std::io::Result<i32> {
    let Some(plugin_id) = args.first() else {
        super::eprint_usage("plugin unlink <plugin_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        super::eprint_usage("plugin unlink <plugin_id>");
...
fn plugin_install(args: &[String]) -> std::io::Result<i32> {
    let Some(source_arg) = args.first() else {
        super::eprint_usage("plugin install <owner>/<repo>[/subdir...] [--ref REF] [--yes]");
        return Ok(2);
    };
...
fn plugin_uninstall(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        super::eprint_usage("plugin uninstall <plugin_id|owner/repo[/subdir...]>");
        return Ok(2);
    };
    if args.len() != 1 {
        super::eprint_usage("plugin uninstall <plugin_id|owner/repo[/subdir...]>");
...
fn plugin_set_enabled(args: &[String], enabled: bool) -> std::io::Result<i32> {
    ...
    let Some(plugin_id) = args.first() else {
        super::eprint_usage(&format!("plugin {action} <plugin_id>"));
        return Ok(2);
    };
    if args.len() != 1 {
        super::eprint_usage(&format!("plugin {action} <plugin_id>"));
...
fn plugin_action_invoke(args: &[String]) -> std::io::Result<i32> {
    let Some(action_id) = args.first() else {
        super::eprint_usage("plugin action invoke <action_id> [--plugin ID]");
        return Ok(2);
    };
    let mut plugin_id = None;
    let mut index = 1;
...
fn plugin_pane_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(pane_id) = args.first() else {
        super::eprint_usage("plugin pane focus <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        super::eprint_usage("plugin pane focus <pane_id>");
...
fn plugin_pane_close(args: &[String]) -> std::io::Result<i32> {
    let Some(pane_id) = args.first() else {
        super::eprint_usage("plugin pane close <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        super::eprint_usage("plugin pane close <pane_id>");
...
impl GithubPluginSource {
    fn parse(value: &str) -> Result<Self, String> {
        ...
        if parts.len() < 2 {
            return Err(crate::identity::usage("plugin install <owner>/<repo>[/subdir...]"));
        }
        let owner = parts[0];
        let repo = parts[1];
