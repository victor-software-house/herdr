...
fn server_stop(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        super::eprint_usage("server stop");
        return Ok(2);
    }
...
fn server_reload_config(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        super::eprint_usage("server reload-config");
        return Ok(2);
    }
...
fn server_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    let json = match args {
        ...
        _ => {
            super::eprint_usage("server agent-manifests [--json]");
            return Ok(2);
        }
    };
...
fn server_reload_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        super::eprint_usage("server reload-agent-manifests");
        return Ok(2);
    }
...
fn server_update_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    let json = match args {
        ...
        [flag] if flag == "--json" => true,
        _ => {
            super::eprint_usage("server update-agent-manifests [--json]");
            return Ok(2);
        }
    };
...
fn server_live_handoff(args: &[String]) -> std::io::Result<i32> {
    let Some(params) = parse_live_handoff_params(args) else {
        super::eprint_usage("server live-handoff [--import-exe <path>] [--expected-protocol <n>] [--expected-version <version>]");
        return Ok(2);
    };
